...
/**
 * A OWL API based reasoner wrapper that supports classification,
 * satisfiability, consistency, and realisation.
 */
public class OREv2ReasonerWrapper {
    ...
    public OREv2ReasonerWrapper(OWLOntology ontology, String reasonerFactoryClass, String errorOutputFileString) {
        mOntology = ontology;
        mLoadReasonerFactoryClass = reasonerFactoryClass;
        mErrorOutputFileString = errorOutputFileString;
    }
    ...
    public Set<OWLAxiom> classify() throws Exception {
        ...
        try {
            ...
            Set<OWLSubClassOfAxiom> subClassAxioms = subClassGenerator.createAxioms(manager.getOWLDataFactory(),
                reasoner);
            Set<OWLEquivalentClassesAxiom> equivClassAxioms = equivClassGenerator.createAxioms(manager
                .getOWLDataFactory(), reasoner);
    ...
    public Set<OWLAxiom> realise() throws Exception {
        ...
        try {
            ...
            OWLDataFactory factory = manager.getOWLDataFactory();
            resultAxioms = Collections.singleton(factory.getOWLSubClassOfAxiom(factory.getOWLThing(), factory
                .getOWLNothing()));
        } catch (Exception e) {
            writeError(e.toString());
            writeError(e.getStackTrace().toString());
     ...
     * 
     * @param axiom
     *        OWL Axiom for which the entailment is checked
    ...
    public String serializeOntologyResults(Set<OWLAxiom> results, OWLOntologyManager manager, String outputFileString)
        ...
        try {
            ...
            Writer w = new OutputStreamWriter(fileOutputStream, Charsets.UTF_8);
            StringDocumentTarget t = new StringDocumentTarget();
            manager.saveOntology(manager.createOntology(results, iri), new FunctionalSyntaxDocumentFormat(), t);
            BufferedReader r = new BufferedReader(new StringReader(t.toString()));
            String l = r.readLine();
            while (l != null) {
                if (!l.startsWith("#")) {
                    // skip comments
    ...
    public static void main(String[] args) throws Exception {
        ...
        OREv2ReasonerWrapper reasonerWrapper = new OREv2ReasonerWrapper(ontology, factoryString, outputFileString
            + "_err");
        if (operationString.equalsIgnoreCase("satisfiability")) {
            ...
            reasonerWrapper.serializeString(outputFileString, satClass.getIRI().toString() + "," + reasonerWrapper
                .isSatisfiable(satClass));
            ...
            reasonerWrapper.serializeOntologyResults(results, OWLManager.createOWLOntologyManager(), outputFileString);
            ...
            OWLOntology entailmentsOntology = managerEntailments.loadOntologyFromOntologyDocument(
                entailmentsOntologyFile.getAbsoluteFile());
            boolean entailed = false;
